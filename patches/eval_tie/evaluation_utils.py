...
def vote(logit_list):
    ...
    if len(indices) > 1:
        # Exact float ties between the top classes used to return the string
        # "tie", which then reached sklearn as a mixed-type label array:
        #   ValueError: Mix of label input types (string and number)
        #   Got [0 1 ... 24] and ['0' ... 'tie']   (confusion_matrix)
        # Break the tie deterministically (lowest class index) and keep the
        # value numeric so the whole metric/prediction path stays consistent.
        return indices[0]
    else:
        return indices[0]
...
def classifier_predict(model, classifier_type, evalset, forward_batch_size, gene_token_dict, predict_metadata=None):
    ...
    for i in trange(0, evalset_len, forward_batch_size):
        ...
        with torch.no_grad():
            outputs = model(
                input_ids=input_data_batch.to(get_device_obj()),
                attention_mask=attn_msk_batch.to(get_device_obj()),
                labels=label_batch.to(get_device_obj()),
            )
