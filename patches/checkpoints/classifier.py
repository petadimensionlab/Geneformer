...
from .device import get_device_obj
...
def _latest_checkpoint(output_directory):
    """Return the newest Trainer checkpoint dir under ``output_directory``.

    Used to resume an interrupted fine-tune instead of restarting from scratch.
    ...
    try:
        ...
        if output_directory and os.path.isdir(output_directory):
            return get_last_checkpoint(output_directory)
    except Exception:  # pragma: no cover - resume is best-effort
        pass
    return None


def _apply_checkpoint_defaults(def_training_args, logging_steps):
    """Default to *periodic step* checkpoints so an interrupted run resumes.

    Geneformer historically hard-coded ``save_strategy="epoch"`` with
    ``save_total_limit=1``, which means a crash/OOM/exit mid-epoch loses every
    completed step (hours of training). We now:

      * keep whatever the caller passed (``setdefault``, not ``update``),
      * default to step checkpoints with a bounded retention,
      * keep ``save``/``eval`` strategies consistent, because Hugging Face
        raises when ``load_best_model_at_end=True`` and the strategies differ.

    Returns the (mutated) training-args dict.
    """
    def_training_args.setdefault("save_strategy", "steps")
    def_training_args.setdefault("save_total_limit", 2)
    if def_training_args.get("save_strategy") == "steps":
        def_training_args.setdefault("save_steps", max(1, int(logging_steps)))
        if def_training_args.get("load_best_model_at_end") and def_training_args.get(
            "eval_strategy", "epoch"
        ) != "steps":
            def_training_args["eval_strategy"] = "steps"
            def_training_args["eval_steps"] = def_training_args["save_steps"]
    return def_training_args
...
class Classifier:
    ...
    def hyperopt_classifier(
        ...
        **Parameters**
        ...
        def_training_args = _apply_checkpoint_defaults(def_training_args, logging_steps)
    ...
    def train_classifier(
        ...
        **Parameters**
        ...
        # train the classifier (resume from the newest step checkpoint if the
        # previous run was interrupted, so completed steps are not lost)
        _resume = _latest_checkpoint(output_directory)
        if _resume:
            logger.info(f"Resuming fine-tuning from checkpoint: {_resume}")
            print(f"Resuming fine-tuning from checkpoint: {_resume}", flush=True)
