...
from .device import empty_cache, get_device_obj
...
def get_embs(
    ...
    for i in trange(0, total_batch_length, forward_batch_size, leave=(not silent)):
        ...
        original_lens = torch.tensor(minibatch["length"], device=get_device_obj())
        ...
        with torch.no_grad():
            outputs = model(
                input_ids=input_data_minibatch.to(get_device_obj()),
...
def label_cell_embs(embs, downsampled_data, emb_labels):
    embs_df = pd.DataFrame(embs.float().cpu().numpy())
        ...
        **Examples:**
        ...
        if self.emb_mode == "cell":
            if self.summary_stat is None:
                ...
                embs_df = pd.DataFrame(embs.float().cpu().numpy()).T
