...
> | 順 | 文書 | 何が書いてあるか |
> |---|---|---|
> | **1** | **[記述ルール](style-guide.md)** | **用語、表記、判定の語彙。書き方の唯一の基準** |
...
>
> 記述が食い違ったときは、勝手に上書きせず [記述ルール §9「競合・重複の記録」](style-guide.md) に追記してください。
...
- **[執筆者名簿とページ担当](owners.md)** — 誰がどのページを書くか（3名のエージェントと Nakaoka さん）。
- **[競合の記録](conflicts.md)** — 3名が同時に書いたために起きた競合と、その解消のしかた。
- **[記述ルール（用語と表現）](style-guide.md)** — **最初にこれ。** 用語の統一表、表現のルール、根拠の書き方。書き方で迷ったときの唯一の基準
