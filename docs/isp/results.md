...
**解釈**: **T 細胞軸（CD3E/CD4）が最有力**。血液では APOE ほぼ中性（脳と対照）。CD8A は強く負（-1.12e-01）= CD8 細胞毒性は WT 維持に寄与。
...
| 段階 | 遺伝子 | Shift | 判定 |
|---|---|---|---|
| Step 1（`AD` → `WT`） | 7 | すべて \|Shift\| ≤ 1.96e-04 | E1'（参考）の 2.1e-04 を下回る。対照遺伝子の ACTB は −1.87e-05 で、対象遺伝子と区別できない（E3 の基準を満たしません） |
| Step 2（`MG1` → `MG2`） | SORL1 | −3.43e-04 | Step 1 の 2.9 倍。状態分離を広げると信号が出る |

出所は `results/isp/step1_mg_all/` と `results/isp/step2_mg1_mg2/`、実行条件は[評価基準](criteria.md)にあります。
