...
### 6.1 DMNX_Neu（16 遺伝子、完了）— `Shift`

値の根拠の種類は **A**（本実行の実測）です。

| 遺伝子 | `Shift` | 検出率 | | 遺伝子 | `Shift` | 検出率 |
|---|---:|---:|---|---|---:|---:|
| **DNAJC13** | **+2.13e-03** | 59.7% | | ATP13A2 | +6.44e-04 | 62.3% |
| **RBFOX3** | **+1.57e-03** | 41.5% | | NEFH | +5.83e-04 | 24.6% |
| **LRRK2** | **+1.48e-03** | 28.2% | | SNAP25 | +5.71e-04 | 33.6% |
| **KCNJ6** | **+1.24e-03** | 47.9% | | PRKN | +3.15e-04 | 37.2% |
| TMEM175 | +9.16e-04 | 37.0% | | NEFM | +2.31e-04 | 33.2% |
| FBXO7 | +8.03e-04 | 29.0% | | PINK1 | +3.94e-05 | 33.6% |
| SNCA | +6.85e-04 | 28.9% | | VPS35 | −1.29e-05 | 36.0% |
| — | — | — | | NEFL | −1.12e-04 | 35.5% |
| — | — | — | | MAPT | −4.82e-04 | 57.1% |

読み方（[用語と手法の定義](glossary.md) §7）: 正は「削除で PD 細胞が健常状態へ近づいた」を意味します。
**これは順位の材料であり、遺伝子名を介入候補として断定するものではありません**（§7 の留保を参照）。

### 6.2 GPI_Neu（15 遺伝子、完了）— `Shift`

値の根拠の種類は **A**（本実行の実測）です。

| 遺伝子 | `Shift` | 検出率 | | 遺伝子 | `Shift` | 検出率 |
|---|---:|---:|---|---|---:|---:|
| **LRRK2** | **+7.00e-04** | 49.9% | | TMEM175 | −4.91e-05 | 38.0% |
| **SNCA** | **+4.69e-04** | 54.8% | | RBFOX3 | −8.73e-05 | 75.4% |
| ATP13A2 | +3.94e-04 | 50.7% | | MAPT | −2.37e-04 | 58.7% |
| PRKN | +2.96e-04 | 41.9% | | KCNJ6 | −4.67e-04 | 42.1% |
| SNAP25 | +1.53e-04 | 37.8% | | CALB1 | −4.75e-04 | 23.2% |
| NEFL | +6.73e-05 | 20.1% | | PINK1 | −8.20e-04 | 32.4% |
| DNAJC13 | +2.11e-05 | 55.4% | | VPS35 | −1.24e-03 | 32.0% |
| — | — | — | | FBXO7 | −1.50e-03 | 21.4% |

全 15 遺伝子で `|Shift| ≤ 1.5e-03`（最大は最下位 FBXO7 の負方向）であり、DMNX_Neu の最上位
（DNAJC13 +2.13e-03）には届きません。**8/15 が負**で、プール全体の移動方向が
DMNX_Neu（13/16 が正）と逆です。

### 6.3 2 領域間の一致度（参考。判定不能）

**この比較は結論に使えません。** 「2 つの細胞種が同じ応答を示す」という前提が
正当化されていないためです。細胞種が違えば応答が違うのは想定内であり、
順位の食い違いは「ノイズの証拠」にはなりません。値は記述統計としてだけ残します。

共通して解析した **14 遺伝子**で両プールの順位を比較しました。根拠の種類は **A**（実測）です。

| 指標 | 値 | 判定 | 理由 |
|---|---|---|---|
| 符号一致 | 8 / 14 | **判定不能** | 偶然の期待値（7/14）と区別できない |
| Spearman ρ | +0.165 (p = 0.573) | **判定不能** | 一致を期待する前提が無い |
| Pearson r | +0.257 (p = 0.376) | **判定不能** | 同上 |
...
| RBFOX3 | 2 | 9 | |
| KCNJ6 | 4 | 11 | |
| **FBXO7** | 6 | **14（最下位）** | **符号も反転**（+8.03e-04 → −1.50e-03） |

**したがって遺伝子名を結論に書きません。** §7 の留保が解消するまで、単一プールの上位遺伝子を
PD の知見として報告しません。

### 6.4 数値の根拠の種類（運用ルール 4）

| 数値 | 根拠 | 出所と条件 |
|---|---|---|
| 分類器の accuracy、macro F1、クラス別 precision と recall | **A** | 本実行の test（`_3`、3,725 細胞、12 クラス） |
| `Shift`、検出率、平均、中央値、IQR | **A** | 本実行の ISP（`pd_atlas_early_isp_stats_combined.csv`） |
| ノイズ床 2.1e-04 | **B**（借用） | AD_spleen・V2-316M・24 遺伝子 × 3 時点 × 100 細胞の実測（`docs/quantization/REPORT-316M-ISP-jp.md`）。**組織が違うため要確認** |
| 相対距離（状態分離） | **A**（定義に基づく計算値） | 式は「距離 ÷ 2 つのベクトルの平均ノルム」という**定義**です（ライブラリに実装は無く、AG-GH が確認）。本実行の値は **DMNX_Neu 8.21%**、**GPI_Neu 52.29%**（状態埋め込み `isp_state_embs_{pool}.pkl`、`max_ncells=300`）。**値は使用したベクトルと `max_ncells` に依存します**。E5 の基準は根拠の種類 **B**（他組織からの借用）なので**要確認**です |
| null 分布の 95 パーセンタイル | **C**（未確立） | 要測定。無関係な遺伝子を 20〜30 個走らせて作る |
