...
> **状態（2026-09-16 時点）**: データ取得・トークナイズ・埋め込み・ベースライン・fine-tune
> まで完了。ISP は **DMNX_Neu 完了 / GPI_Neu 実行中**（下記 §6 に進捗を明記）。
> fine-tune は **部分 epoch（31%）** であり、§4・§7 の留保を読んでから数値を引用してください。
...
遺伝子は実行前に**プール内 PD 細胞での発現率 ≥20%** を確認（低発現遺伝子は
...
**実行中**（RBFOX3, DNAJC13, … 進行）。完了後に本節を更新します。
対象 15 遺伝子: RBFOX3, MAPT, DNAJC13, SNCA, ATP13A2, LRRK2, KCNJ6, PRKN, TMEM175,
SNAP25, PINK1, VPS35, CALB1, FBXO7, NEFL（発現率順）。

## 7. 解釈の留保（重要）

1. **絶対値が小さい。** DMNX_Neu の `Shift` は最大 1.4e-3（DNAJC13 2.1e-3）で、
   リポジトリが bf16 のノイズフロアとして挙げる **|Shift| ≈ 2e-4** の 5〜10 倍程度に
   すぎません。**単独では生物学的結論を出せません。**
2. **有意性検定は未実施。** 順位を主張するには
   `analysis/07d_in_silico_perturbation_PD_null.py` 相当の**並べ替え null 比較**が必要です。
3. **分類器の偏り（§5）。** ISP の 2 プールは分類器が最も苦手なクラスであり、
   GPI_Neu の recall は 0.663 です。さらに **fine-tune は 31% epoch** です。
4. **バッチサイズは結果に影響しません。** ISP の padding は `mean_nonpadding_embs` で
...
- [ ] **フル epoch で再学習**（`FINETUNE_MAX_STEPS=0`、約 10.4 時間）。`patches/checkpoints/classifier.py`
      適用後は**中断してもチェックポイントから自動再開**できます
- [ ] **null 比較**（順位の有意性）
- [ ] 他領域への拡張（PVC を対照に PMC/PFC と比較）
