...
> **本実行から遺伝子を結論として引用しないでください。** null 比較で **E2 が不合格**でした
...
|---|---:|---:|---:|---:|---|
| DMNX_Neu | 0.454 | 7.39e-04 | 7.58e-04 | 0.675 | **不合格** |
| GPI_Neu | 0.498 | 4.65e-04 | 4.61e-04 | 0.517 | **不合格** |
...
[評価基準](criteria.md) §2-1-2 の裁定（「確かめる対象が存在しないため、合格とも不合格とも書かない」）に従い
**未検証**とします。

#### 考察

- **E1 の基準は確立しました**（DMNX_Neu 1.34e-03、GPI_Neu 1.25e-03、根拠 A）。これで「どの値なら意味があるか」をデータから言えます
- **しかし基準を超えたのは 3/16 と 1/15 にとどまり、E2 は不合格**（AUC 0.454 / 0.498）です。
...
- **限界**: null は 15 個で [評価基準](criteria.md) の 20〜30 個に届きません。また null の一部
...
2. **null 比較で、無関係な遺伝子と区別できませんでした（最も重い留保）。** 本データで E1 の基準を
   実測しました（DMNX_Neu **1.34e-03**、GPI_Neu **1.25e-03**、根拠 A）。しかし基準を超えたのは
   **3 / 16** と **1 / 15** にとどまり、効果量は **AUC 0.454 / 0.498**（E2 **不合格**）です。
   **無関係な 15 遺伝子が、PD 遺伝子と同じ大きさの変位を出しています**（§6.5）。
3. **陰性対照は測定できませんでした（E3 未検証）。** `ACTB`（検出率 18.1%）と `GAPDH`（15.8%）は
   規則を満たさず、このデータでは摂動できません。**指標そのものの妥当性は確かめられていません。**
