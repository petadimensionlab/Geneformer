...
3. **4bit (nf4) は精度が落ちます**。今回の測定では採用できません。
   - 埋め込みコサイン 0.981、遺伝子ランキングの上位 100 件の一致が 87/100
...
| 方式 | MLM loss | マスク予測 正解率 | 埋め込み cos 平均 | cos 最小 | 遺伝子の上位 100 件の一致 |
|---|---|---|---|---|---|
...
| G3 | セル埋め込みの cos | 平均 0.9995 以上・最小 0.999 以上 | int8 は基準を満たし、nf4 は満たさない |
| G4 | プローブ（celltype 分類） | 正解率・macro-F1 の差が 0.01 以内 | 下流タスクでの実害を見ます |
| G5 | **ISP の順位一致**（`Shift_to_goal_end`） | スピアマンの順位相関 0.99 以上、上位 20 件の一致 18/20 以上、符号の一致 98% 以上 | **最重要**。生物学的な結論を守るため |
| G6 | 微調整済み分類器の評価値 | macro-F1 の差が 0.01 以内 | 結論に直結します |
| G7 | 少数派の最悪ケース | 希少な celltype や短いセル（100トークン未満）でも cos 0.99 以上 | 平均では隠れる劣化を探すため |

> **G5 の基準は実測にもとづいて改訂済みです**（[REPORT-316M-ISP-jp.md](REPORT-316M-ISP-jp.md) §3）。
> `Shift_to_goal_end` は約1のコサイン同士の差なので桁落ちで有効数字が2〜3桁しかなく、
> スピアマンの順位相関 ≥ 0.99 は原理的に達成できません。改訂後の基準は **上位 N 件の一致 ≥ 18/20、スピアマンの順位相関 ≥ 0.98、
> ノイズフロア（3 × 中央値|Δ|）以上の遺伝子での符号一致 100%** です。
> bf16 の実測値は、スピアマンの順位相関 = 0.9839、上位 20 件の一致 20/20、ノイズフロア上の符号の一致 100% で、改訂案の基準を満たしました。
...
- 比較対象は既存の fp32 微調整済み `ksplit1`（6組織）で、macro-F1 の差 0.01 以内を基準とします。
...
5. **nf4 の劣化は平均ではなく順位に出ます**（上位 100 件の一致が 87/100）。
