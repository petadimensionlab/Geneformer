#!/usr/bin/env python
"""In silico perturbation (delete) for early Parkinson's detection in lymph node.

Target: lymph-node APC/monocyte cell types; model the early pathological shift
   start = WT 6m  (healthy baseline)
   goal  = PF 6m  (alpha-synuclein preformed-fibril seeding = early pathology)
   alt   = mo 6m / tgPD 6m

For each of the 6 APC cell types we run the tutorial's three steps:
...
Design choices / ported learnings from 07_in_silico_perturbation.py:
- MPS host; datasets must be <5 (datasets==4.0.0) or perturb_data's dataset.map
  hangs.  We detect and warn.
...
IMPORTANT about per-gene vs. group perturbation:
  A fixed *list* in genes_to_perturb is treated as a GROUP deletion: cells must
  express ALL listed genes at once, and goal_state_shift then returns ONE
  averaged row (it reads only cos_sims_df['Gene'][0]).  That is not per-gene.
  To rank individual genes we therefore delete ONE gene at a time
  (genes_to_perturb=[gene]); a single-gene list is an individual deletion.

CRITICAL: each gene's perturbation must live in its OWN subdirectory.
  InSilicoPerturberStats.read_dictionaries reads EVERY *_raw.pickle in the
  directory it is given.  If all genes shared one directory, each per-gene
  stats call would merge all genes and collapse to the same Gene[0] value.
  Isolating each gene under isp_dir/<celltype>/<gene>/ makes each stats call
  see only that gene, yielding its own Shift_to_goal_end.
"""
...
TARGET_GENES = [
    ...
    # expanded: monocyte/microglia & PD-risk immune genes (option 4)
...
# Target cell types (APC / monocyte lineage; early immune sensing).
APC_CELLTYPES = [
    "Migratory.DCs",
    "cDC.1",
    "cDC.2",
...
def build_early_dataset(tokenized_path: Path, early_path: Path, nproc: int) -> Path:
    ...
    Kept separate from the shared PD_LN.dataset so 07_in_silico_perturbation.py
...
def main() -> None:
    ...
    # optional subset for a validation run, e.g. IS_CTYPES="Migratory.DCs"
    ...
    summary = ISP_DIR / "pd_apc_early_isp_summary.csv"
    if summary.exists():
        summary.unlink()
    ...
    print("=== PD EARLY IS PERTURBATION DONE ===", flush=True)
