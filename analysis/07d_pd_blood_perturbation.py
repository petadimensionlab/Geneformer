#!/usr/bin/env python
"""In silico perturbation (delete) for early Parkinson's detection in BLOOD.

Adapted from 07b_pd_perturbation.py (lymph node) and 07c_pd_bm_perturbation.py
(bone marrow) for PD_blood (mouse alpha-synuclein PFF-seeding model).

Target: blood monocyte / APC / adaptive immune cell types; model the early
pathological shift at the 6-month timepoint
   start = WT 6m  (healthy baseline)
   goal  = PF 6m  (alpha-synuclein preformed-fibril seeding = early pathology)
   alt   = mo 6m / tgPD 6m

Cell types (primary + adaptive immune arm):
   Ly6c.high.classical.Monocytes, Ly6c.low.nonclassical.Monocytes,
   Neutrophils, cDCs, pDCs, CD8.T.cells, CD4.T.cells, NK_ILC1

Genes: the shared PD/immune target list PRUNED to genes actually expressed in
blood (GBA1, HLA-DRA, HLA-DRB1, CX3CR1, FCGR3A are unexpressed in every WT6m
blood cell type and would fail the "no cells contain gene" check).
...
Design choices / ported learnings from 07b/07c:
...
exist on this host.  Use the real local fine-tuned CellClassifier:
   input/PD_blood/runs/260823_geneformer_cellClassifier_PD_blood_celltype/ksplit1
...
# Pruned for blood: GBA1, HLA-DRA, HLA-DRB1, CX3CR1, FCGR3A are NOT expressed
# in any WT6m blood cell type (deleting them would raise
# "no cells contain genes_to_perturb").
TARGET_GENES = [
    # PD core (GBA1 dropped: unexpressed in blood)
    "ENSG00000145335",  # SNCA
    "ENSG00000188906",  # LRRK2
    "ENSG00000069329",  # VPS35
    "ENSG00000116288",  # PARK7
    # immune sensors / antigen presentation (HLA-DRA/HLA-DRB1 dropped)
    ...
    # (CX3CR1, FCGR3A dropped: unexpressed in blood)
    ...
    "ENSG00000172243",  # CLEC7A (Dectin-1)
    "ENSG00000143226",  # FCGR2A
    "ENSG00000177575",  # CD163
]

# Target cell types (primary immune sentinels + adaptive immune arm).
BLOOD_CELLTYPES = [
...
# to a Linux path (input/PD_blood/runs/.../ksplit1 is what actually exists here).
LOCAL_CELLCLASSIFIER = (
    Path(os.environ.get(
        "ADPD_ROOT",
        Path(__file__).resolve().parent.parent / "input" / "PD_blood",
    ))
    / "runs" / "260823_geneformer_cellClassifier_PD_blood_celltype" / "ksplit1"
)
...
def build_early_dataset(tokenized_path: Path, early_path: Path, nproc: int) -> Path:
    ...
    Kept separate from the shared PD_blood.dataset so 07_in_silico_perturbation.py
...
def main() -> None:
    ...
    # optional subset for a validation run, e.g. IS_CTYPES="cDCs,Neutrophils"
    ctypes = BLOOD_CELLTYPES
    ...
    summary = ISP_DIR / "pd_blood_apc_early_isp_summary.csv"
    if summary.exists():
        summary.unlink()
    ...
    print("=== PD BLOOD EARLY IS PERTURBATION DONE ===", flush=True)
