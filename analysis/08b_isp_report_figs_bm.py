#!/usr/bin/env python
"""Visualize the early-PD in silico perturbation results for BONE MARROW.

Reads the per-gene goal_state_shift summary (result/report_bm/..._full6.csv) and
writes PNG figures under result/report_bm/assets/:
...
A consensus gene-ranking table (isp_gene_ranking.csv) is also emitted.

Usage:
  .venv/bin/python3 analysis/08b_isp_report_figs_bm.py
"""
...
OUT = _REPO / "result" / "report_bm" / "assets"
SRC = _REPO / "result" / "report_bm"
os.makedirs(OUT, exist_ok=True)


def main() -> None:
    df = pd.read_csv(SRC / "pd_bm_apc_early_isp_summary_full6.csv")
    for c in ("Shift_to_goal_end", "Shift_to_alt_end_mo", "Shift_to_alt_end_tg"):
    ...
    plt.rcParams.update({
        ...
        "font.sans-serif": ["DejaVu Sans", "Hiragino Sans", "Yu Gothic"],
    })
    ...
    sns.heatmap(hp, ax=ax, cmap=cmap, center=0,
                ...
                cbar_kws={"label": "Shift_to_goal_end\n(deletion of gene)",
                          "shrink": 0.8})
    ax.set_title("In silico deletion (bone marrow): shift of WT cells toward early-PD (PF6m)\n"
    ...
    ax.set_xlabel("Mean Shift_to_goal_end across bone-marrow APC cell types")
    ax.set_title("Bone marrow: deletion of these genes moves WT cells toward early-PD (PF6m)\n"
    ...
    fig.suptitle("Per-cell-type in silico deletion → early-PD shift (bone marrow)", fontsize=11)
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    fig.savefig(OUT / "isp_celltype_volcano.png")
    plt.close(fig)

    # ---- gene ranking table ----
    agg.round(6).to_csv(OUT / "isp_gene_ranking.csv", index=False)
